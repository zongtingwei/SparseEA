function [Population, FrontNo, CrowdDis] = EnvironmentalSelection(Population, N)
    ...
    [uniqueDecs, index] = unique([Population.decs], 'rows', 'stable');
    Population = Population(index);

    % Non-dominated sorting
    [FrontNo, MaxFNo] = NDSort([Population.objs], inf);
    NextNo = false(1, size(Population, 1));
    ...
    if sum(PopObj(:, 2) <= minobj2) >= N
        ...
        Population = Population(boolFeasible);
...
function selectedNo = DuplicationSelection(index, Population, NDpoints)
    ...
    c1 = find(min(abs(Obj(:, 1) - NDobj(:, 1))) == abs(Obj(:, 1) - NDobj(:, 1)));
    c2 = find(min(abs(Obj(:, 2) - NDobj(:, 2))) == abs(Obj(:, 2) - NDobj(:, 2)));
    o1 = [];
    for i = 1:size(c1, 1)
